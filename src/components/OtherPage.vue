      ...
      <v-card max-width="600" class="mx-auto">
        ...
        <v-container>
          <v-col cols="12">
            <v-card color="#f0f8ff" theme="dark">
              <div class="d-flex flex-no-wrap justify-space-between">
                ...
                <v-card-actions>
                  <v-btn class="ma-2" variant="outlined" width="75" height="75" color="white"
                    style="font-weight: bold;background-color: #4169e1;"
                    @click="() => $router.push({ name: 'interview1', query: { name: text } })">面談に<br>進む


                    <v-icon size="large" icon="mdi-arrow-right-bold-box-outline"></v-icon>
                  </v-btn>
                </v-card-actions>
              </div>
            </v-card>
          ...
          <v-btn color="success" class="mt-4" block
            @click="() => $router.push({ name: 'finish', query: { name: text } })">面談終了
          </v-btn>
